...
def move(state, direction):
    ...
    if type(state) != tuple:
        raise UserWarning(
            "@@A test provides a non-tuple as first argument. The resulting behavior is undefined.@@")

    if len(state) and not (type(state[0]) == str):
        raise UserWarning(
            "@@A test uses a tuple with non-string elements as first argument. The resulting behavior is undefined.@@")
